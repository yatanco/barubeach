...
export const PATCH: APIRoute = async ({ request, locals, params }) => {
  let body: { notes?: string; guest_phone?: string };
  ...
  if (typeof body.notes === 'string') {
    await db.prepare('UPDATE bookings SET notes = ?1, updated_at = ?2 WHERE id = ?3').bind(body.notes, now, params.id).run();
  }
  if (typeof body.guest_phone === 'string') {
    await db.prepare('UPDATE bookings SET guest_phone = ?1, updated_at = ?2 WHERE id = ?3').bind(body.guest_phone.trim() || null, now, params.id).run();
  }
