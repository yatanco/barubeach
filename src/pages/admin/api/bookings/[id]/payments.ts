...
// Section 7 (Payments received) for bookings. New payments anchor to a
// find-or-created zero-amount 'extra' charge (same pattern as leads), but
...
export const POST: APIRoute = async ({ request, locals, params }) => {
  ...
  const now = nowIso();
  let charge = await db.prepare(`SELECT id FROM charges WHERE booking_id = ?1 AND category = 'extra' AND description = 'Payments received' LIMIT 1`)
