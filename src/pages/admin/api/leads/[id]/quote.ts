...
export const POST: APIRoute = async ({ request, locals, params }) => {
  ...
  await requireDb(locals).prepare(`UPDATE leads SET
      ...
      quote_food = ?5, quote_total = ?6, quote_deposit = ?7, quote_saved_at = ?8, updated_at = ?8
    WHERE id = ?9`)
    .bind(transportIncluded, foodIncluded, accommodation, transport, food, total, deposit, nowIso(), params.id)
    .run();
