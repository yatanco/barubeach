...
- **Schema**: `migrations/0001_crm.sql` defines `leads`, `bookings`, `charges`, `payments`; later migrations extend it — `migrations/0010_unified_pipeline.sql` is the most structurally significant (unified statuses, flat food/transport/accommodation fields, dropped `payment_links`/`provider_payments`). Treat this schema as fixed unless explicitly asked to migrate it — the `status`/`category`/`operational_status`/`channel`/`source`/`guest_intent` columns are `CHECK`-constrained, so any new value needs a migration, not just a code change.
  - **Unified status pipeline** (`UNIFIED_STATUSES` in `src/lib/crm.ts`, shared by `leads.status` and `bookings.status`): `new`, `quoted`, `deposit_requested`, `deposit_paid`, `confirmed`, `upsell_pending`, `upsell_confirmed`, `in_house`, `balance_requested`, `completed`, `lost`, `cancelled`. `DIRECT_PIPELINE` (used for leads and direct/booking.com bookings) includes all of these except `upsell_pending`/`upsell_confirmed`; `AIRBNB_PIPELINE` starts at `confirmed` and skips the pre-booking lead stages. `bookings.status` also still accepts the legacy value `checked_in` — `hosthub-sync.ts`'s date-derived `deriveStatus()` writes it directly and is channel-agnostic, so it can land on any channel, not just Airbnb; `normalizeStatus()` treats it everywhere as a display synonym for `in_house`.
...
- **Detail page**: `src/components/admin/UnifiedRecordDetail.astro` is the single component rendering both `/admin/leads/[id]` and `/admin/bookings/[id]` (each is a thin wrapper that fetches the row and redirects if missing). It renders 10 sections — header/WhatsApp/phone, status pills, accommodation, food & transport, totals, collect deposit (copies a WhatsApp message with an evergreen Bold payment link), payments received, collapsed variable costs (bookings only), notes, and a HostHub card (channel + link-to-HostHub for bookings, a "link reservation → confirm booking" form for leads that converts them via `POST /admin/api/bookings/create`).
- **Dashboard** (`src/pages/admin/index.astro`): summary line, an "Arriving this week" card, one unified list (no separate leads/bookings sections) sorted by check-in date with undated leads appended by `created_at DESC`, filter tabs (All/Bookings/Leads/Deposit Pending/In House/Upcoming), and a per-row status badge that opens a small popover to change status inline via `PATCH` — no navigation needed. A lead that's already been converted to a booking is excluded from the list (the booking is its continuation).
